...
function main(options) {
    return (tree, _context) => {
            ...
            (0, schematics_1.forEach)((fileEntry) => {
                ...
                if (fileName.endsWith('.entity.ts')) {
                    return {
                        ...
                        path: (0, core_1.normalize)(`entities/${options.entityName}.entity.ts`)
                    };
                }
        ...
        const updateModule = (options.updateParentModule === false)
            ? (0, schematics_1.noop)()
            : isTopLevelModule
                ? (tree) => updateAppModule(tree, options)
...
function updateAppModule(tree, options) {
    return (tree) => {
        ...
        let appModuleContent = (_a = tree.read(appModulePath)) === null || _a === void 0 ? void 0 : _a.toString('utf-8');
        if (!appModuleContent) {
            return tree;
        }
        // Create module name
        const currentModule = options.moduleName;
        const moduleName = core_1.strings.classify(`${currentModule}Module`);
        const importPath = `./modules/${core_1.strings.dasherize(options.name)}/${core_1.strings.dasherize(currentModule)}.module`;
        ...
        appModuleContent = appModuleContent.replace(/imports\s*:\s*\[([\s\S]*?)\],/, (match, importContents) => {
 ...
 */
function updateModuleFile(tree, options) {
    return (tree) => {
        var _a;
        ...
        const modulePath = `./${core_1.strings.dasherize(currentModule)}/${core_1.strings.dasherize(currentModule)}.module`;
        // Read and parse module file as text (easier than AST manipulation)
        let moduleContent = (_a = tree.read(parentModulePath)) === null || _a === void 0 ? void 0 : _a.toString('utf-8');
        if (!moduleContent) {
            return tree;
        }
        // Add import statement if not already present
        ...
        // Add module to imports array with consistent formatting
        moduleContent = moduleContent.replace(/imports\s*:\s*\[([\s\S]*?)\],/, (match, importContents) => {
            // Extract all current imports
            const importItems = importContents
                .split(',')
                .map(item => item.trim())
        ...
        moduleContent = moduleContent.replace(/exports\s*:\s*\[([\s\S]*?)\],/, (match, exportContents) => {
        ...
        moduleContent = reconstructRouterModule(moduleContent, segments, currentModule, moduleName);
        // Write modified content back to file
 ...
 */
function reconstructRouterModule(content, pathSegments, currentModule, moduleName) {
    ...
    return content.replace(routerMatch[0], `RouterModule.register([\n${formattedRoutes}\n        ]),`);
}
/**
 * Parse route objects from RouterModule configuration
 */
function parseRoutes(routerConfig) {
    const routes = [];
 ...
 */
function addRouteToParent(routes, pathSegments, currentModule, moduleName) {
    ...
    if (pathSegments.length === 1) {
        const parentPath = core_1.strings.dasherize(pathSegments[0]);
        ...
        if (parentIndex >= 0) {
            ...
            const childExists = parent.children.some(c => c.path === core_1.strings.dasherize(currentModule));
            if (!childExists) {
                // Add new child to parent
                parent.children.push({
                    path: core_1.strings.dasherize(currentModule),
                    module: moduleName
                });
            }
        }
    ...
    const topLevelPath = core_1.strings.dasherize(pathSegments[0]);
    ...
    for (let i = 1; i < pathSegments.length; i++) {
        const segmentPath = core_1.strings.dasherize(pathSegments[i]);
        // Initialize children array if needed
        ...
        if (!found) {
            ...
            const segmentModuleName = core_1.strings.classify(`${pathSegments[i]}Module`);
            // Add new child route
            const newRoute = {
                path: segmentPath,
    ...
    // Check if child already exists
    const childExists = currentParent.children.some(c => c.path === core_1.strings.dasherize(currentModule));
...
*/
function formatRoutes(routes, indent = 12) {
